...
def wrapper_feature_selector(X,Y,model,subset=np.arange(0,9).tolist()):
...
def train_and_predict(X,Y,model,n_validation=214,subset=np.arange(0,9).tolist(),predict=True,return_y=True):
...
def fine_tune_KNN(X,Y,subset=np.arange(0,9).tolist()):
    best_rmse = 1e10
    best_k = -1
    best_p = -1
